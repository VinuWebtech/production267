...
class Magebright_Productrestriction_Model_Importproductsku extends Mage_Core_Model_Config_Data
{ 
              ...
              if($collection->getSize() > 0){
                ...
                if($isExist==1){
                   $new_data = array();
                    ...
                    $new_data['sku'] = str_replace('"','',trim(preg_replace('/\s+/',' ', $actual_data[1])));
                    $new_data['product_id'] = str_replace('"','',trim(preg_replace('/\s+/',' ', $actual_data[2])));
                    if($new_data['product_id'] == '')
                    {
                      $_sku = $new_data['sku'];
                      $_catalog = Mage::getModel('catalog/product');
                      $_productId = $_catalog->getIdBySku($_sku);
                      $new_data['product_id'] = $_productId;
                    }
                $query .= " (
                  '".$new_data['pin_code']."',
                  '".$new_data['sku']."',
                ...
                if($rowNumberCount<500):
                   ...
                   if($rowNumber==$rowNumberCount+1):
          		    $final_query = "INSERT into ".$tablename." (pin_code,sku,product_id) ".trim($query,',');
